...
async def create_exportacao(create_exportacao: ExportacaoIn):
...
async def read_exportacao(
    ...
    description_type: Annotated[str | None, Query(description="Query for specific type of product")] = None):
    async with SessionLocal() as session:
        exportacao_objs = await Exportacao.get_all(session, year=year, pais=pais, description_type=description_type)
...
async def update_exportacao_object(id: Annotated[int, Path(title="The ID of the item to get")], exportacao_update: ExportacaoUpdate):
    async with SessionLocal() as session:
